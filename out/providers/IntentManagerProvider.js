...
class FileStat {
    constructor(id, ctime, mtime, size, signed, version, content) {
...
const DECORATION_SIGNED = new vscode.FileDecoration('🔒', 'Signed', new vscode.ThemeColor('list.deemphasizedForeground'));
const DECORATION_UNSIGNED = new vscode.FileDecoration('', 'Unsigned', new vscode.ThemeColor('list.highlightForeground'));
const DECORATION_YANG = new vscode.FileDecoration('☯', 'YANG modules', new vscode.ThemeColor('list.warningForeground'));
const DECORATION_RESOURCE = new vscode.FileDecoration('❯', 'Resources', new vscode.ThemeColor('list.warningForeground'));
const DECORATION_VIEWS = new vscode.FileDecoration('⚯', 'Resources', new vscode.ThemeColor('list.warningForeground'));
const DECORATION_ALIGNED = new vscode.FileDecoration('✔', 'Aligned', new vscode.ThemeColor('list.highlightForeground'));
const DECORATION_MISALIGNED = new vscode.FileDecoration('✘', 'Misaligned', new vscode.ThemeColor('list.errorForeground'));
const DECORATION_NONACTIVE = new vscode.FileDecoration('✘', 'Inactive', new vscode.ThemeColor('list.deemphasizedForeground'));
let myStatusBarItem;
...
class IntentManagerProvider {
    ...
    */
    ...
    async getNSPversion() {
        ...
        if (this.nsp_version === "") {
            ...
            let response = await this._callNSP(urlversion, {
                method: "GET",
                headers: {
                    'Content-Type': 'application/json',
    ...
    */
    async _createIntentFile(name, data) {
        let body = {};
        ...
        let response = await this._callNSP(url, {
            method: method,
            headers: {
    ...
    */
    async _updateIntentType(name, data) {
    ...
    */
    async _getWebviewContent(intent, intenttype, result, panel) {
    ...
    */
    async _getWebviewContentLogs(intent, intenttype, result, panel) {
    ...
    */
    ...
    exposeIntentStatus(documentPath) {
    ...
    */
    async updateIntentNetworkStatus() {
        let items = [];
    ...
    */
    async writeIntentType(name, data) {
    ...
    */
    async createIntentFromScratch(name, uris) {
        var fs = require('fs');
    ...
    */
    async deleteIntentVersion(name) {
        const intent = name.toString().split("/")[2];
    ...
    */
    async createNewIntentVersion(name) {
        const intent = name.toString().split("/")[2];
    ...
    */
    async cloneIntent(name, newName) {
    ...
    */
    async getLogs() {
        ...
        let json = await response.json();
        const panel = vscode.window.createWebviewPanel('intentLogs', intent + ' Logs', vscode.ViewColumn.Two);
        ...
        if (Object.keys(json["ibn:output"]["audit-report"]).includes("misaligned-attribute") || Object.keys(json["ibn:output"]["audit-report"]).includes("misaligned-object")) {
            vscode.window.showWarningMessage("Intent Misaligned", "Details", "Cancel").then(async (selectedItem) => {
                if ('Details' === selectedItem) {
                    ...
                    const panel = vscode.window.createWebviewPanel('auditReport', intent + ' Audit', vscode.ViewColumn.Two, { localResourceRoots: [vscode.Uri.file(path.join(this.extContext.extensionPath, 'media'))] });
    ...
    async readDirectory(uri) {
        ...
        if ((uri.toString() === "im:/intent-types") || (uri.toString() === "im:/intents")) {
            ...
            if (Object.keys(json["ibn:output"]["intents"]).includes("intent")) {
                ...
                result = (json["ibn:output"]["intents"]["intent"] ?? []).map(entry => [entry.target, vscode.FileType.File]);
            }
        }
        console.log(result);
        return result;
...
//# sourceMappingURL=IntentManagerProvider.js.map